...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      background: '#0b1d22',
      border: '1px solid rgba(123,157,174,0.2)',
      borderRadius: 8,
      padding: '10px 14px',
    }}>
      <p style={{ color: 'var(--text-muted)', fontSize: 11, marginBottom: 8, fontWeight: 600 }}>{label}</p>
      <p style={{ color: '#A63A3F', fontSize: 13, marginBottom: 4 }}>Fake: {payload[0]?.value}</p>
      <p style={{ color: '#C6A75E', fontSize: 13, marginBottom: 4 }}>Real: {payload[1]?.value}</p>
...
const FakeRateChart = () => {
  ...
  return (
    <div style={{ marginBottom: '40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '24px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(232,220,200,0.08)', borderRadius: '16px', padding: '20px 24px', display: 'flex', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: 4 }}>Total voice samples</div>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(232,220,200,0.08)', borderRadius: '16px', padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(122,46,50,0.1)', color: '#A63A3F', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
            <i className="ti ti-alert-triangle"></i>
          </div>
          <div>
            <div style={{ fontSize: '32px', fontWeight: 800, color: '#A63A3F', lineHeight: 1 }}>
              <AnimatedNumber value={stats.fake} />
            </div>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: 4 }}>Fake ({stats.fakePct}%)</div>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(232,220,200,0.08)', borderRadius: '16px', padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: 4 }}>Real ({stats.realPct}%)</div>
          </div>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(232,220,200,0.08)', borderRadius: '16px', padding: '20px', display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: 4 }}>Avg Confidence</div>
          </div>
        </div>
      ...
      {/* Chart Card */}
      <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(232,220,200,0.08)', borderRadius: '16px', padding: '24px', backdropFilter: 'blur(16px)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
          <div>
            ...
            <div style={{ fontSize: '16px', color: 'var(--text-main)' }}>
            ...
            {['7 Days', '30 Days', 'All Time'].map(t => (
              <button 
                ...
                style={{ 
                  ...
                  background: timeframe === t ? 'rgba(232,220,200,0.1)' : 'transparent',
              ...
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
                  <linearGradient id="colorFake" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#A63A3F" stopOpacity={0.4}/>
                    <stop offset="95%" stopColor="#A63A3F" stopOpacity={0}/>
                  </linearGradient>
                ...
                <XAxis dataKey="dateStr" stroke="var(--text-muted)" fontSize={11} tickLine={false} axisLine={false} />
                <YAxis stroke="var(--text-muted)" fontSize={11} tickLine={false} axisLine={false} />
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(232,220,200,0.05)" vertical={false} />
                <Tooltip content={<CustomTooltip />} />
                <Area type="monotone" dataKey="fake" name="Fake" stroke="#A63A3F" strokeWidth={3} fillOpacity={1} fill="url(#colorFake)" />
                <Area type="monotone" dataKey="real" name="Real" stroke="#C6A75E" strokeWidth={3} fillOpacity={1} fill="url(#colorReal)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
